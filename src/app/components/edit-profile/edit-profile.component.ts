...
export class EditProfileComponent implements OnInit, AfterViewChecked {
        ...
        .end((_, response) => {
          ...
          if (response && response.statusCode == 200) {
            ...
            this.selectNationality(
              this.sessionData?.nationality || this.nationalities[0]
            );
          } else {
            this.nationalities = [];
          }
      ...
      superagent
        ...
        .end((_, response) => {
          ...
          if (response && response.statusCode == 200) {
            ...
            resolve(null);
            this.selectCity(this.sessionData?.home_city || this.cities[0]);
          } else {
            this.cities = [];
          }
        });
  ...
  resetToDefault() {
    this.displayNameField.nativeElement.value = this.sessionData.display_name;
    this.biographyField.nativeElement.value = this.sessionData.biography;

    this.nationalitySelect.nativeElement.value = this.sessionData.nationality;
    if (!this.sessionData.nationality && this.nationalities.length) {
      this.nationalitySelect.nativeElement.value = this.nationalities[0];
      this.selectNationality(this.nationalities[0]);
    }

    this.homeCitySelect.nativeElement.value = this.sessionData.home_city;
    if (!this.sessionData.home_city && this.cities.length) {
      this.homeCitySelect.nativeElement.value = this.cities[0];
      this.selectCity(this.cities[0]);
    }

    this.occupationSelect.nativeElement.value = this.sessionData.occupation;
    if (!this.sessionData.occupation && this.occupations.length) {
  ...
  saveChangesAndClose() {
    ...
    superagent
      .patch(
        [environment.accounts, this.sessionData.username, 'update'].join('/')
      )
      .send(changes)
      .set(
        'Authorization',
